...
import { AUTH_CODE, DURATION_EXPIRE_TIME, ENABLE_AUTH, DEFAULT_DEV_USER_ID } from "@/constants/auth";
...
export async function generateAdminUserToken() {
  const adminUserToken = await generateToken({
    ...
    email: "admin@omnipost.app",
    role: process.env.SCHEMA_ADMIN_USER || "app20250904195901yvsuhcayno_v1_admin_user",
...
export async function verifyToken(
  ...
  if (!ENABLE_AUTH) {
    // Development mode - return mock valid token
    return {
      valid: true,
